...
Requête : SELECT id_service, nom FROM service;
...
Requête : SELECT COUNT(*) AS nombre_de_patients FROM patient;
...
Requête : SELECT COUNT(*) AS nb FROM chambre WHERE capacite > 1;
...
Requête : SELECT profession, ROUND(AVG(salaire), 2) AS 'salaire moyen' 
          FROM personnel 
          GROUP BY profession 
...
Requête : SELECT salaire * 12 AS salaire_annuel FROM personnel WHERE id_personnel = 13 AND prenom = 'Ophelie';
...
Requête : SELECT * FROM association_patient_chambre 
          WHERE date_entree <= '2011-02-15' AND date_sortie >= '2011-02-01';
...
Requête : SELECT prenom FROM personnel WHERE id_service = '1';
...
Requête : SELECT id_chambre FROM association_patient_chambre 
          WHERE id_patient = (SELECT id_patient FROM patient WHERE prenom = 'elodie');
...
Requête : SELECT nom FROM service 
          WHERE id_service = (SELECT id_service FROM chambre 
          WHERE id_chambre = (SELECT id_chambre FROM association_patient_chambre 
          WHERE id_patient = (SELECT id_patient FROM patient WHERE prenom = 'elodie')));
...
Requête : SELECT s.nom, c.id_chambre, c.capacite 
          FROM chambre c 
          JOIN service s ON c.id_service = s.id_service 
          WHERE s.nom = 'neurologie';
...
Requête : SELECT p.prenom, a.id_patient, a.id_chambre, a.date_entree, a.date_sortie
          FROM patient p
          JOIN association_patient_chambre a ON p.id_patient = a.id_patient
          WHERE a.date_entree <= '2011-02-15' AND a.date_sortie >= '2011-02-01';



18. Un lit a été ajouté dans toutes les chambres du service « neurologie ». Modifiez leur capacité.
Requête : UPDATE chambre SET capacite = capacite + 1 
          WHERE id_service = (SELECT id_service FROM service WHERE nom = 'neurologie');
...
Requête : SELECT s.nom, COUNT(p.prenom) 
          FROM personnel p
          JOIN service s ON p.id_service = s.id_service
          WHERE p.profession = 'infirmier'
          GROUP BY s.nom;
...
Requête : SELECT p.prenom, p.nom, p.profession 
          FROM personnel p 
          WHERE p.profession = 'medecin' 
          AND p.id_service = (
            SELECT c.id_service 
            FROM chambre c 
            JOIN association_patient_chambre apc ON c.id_chambre = apc.id_chambre 
            WHERE apc.id_patient = (SELECT id_patient FROM patient WHERE prenom = 'charlotte')
);
...
Requête : SELECT pa.nom, pa.prenom, s.nom AS service 
          FROM patient pa
          JOIN association_patient_chambre apc ON pa.id_patient = apc.id_patient
          JOIN chambre c ON apc.id_chambre = c.id_chambre
          JOIN service s ON c.id_service = s.id_service;
...
Requête : SELECT pa.* 
          ...
          JOIN association_patient_chambre apc ON pa.id_patient = apc.id_patient
          JOIN chambre c ON apc.id_chambre = c.id_chambre
          WHERE c.id_service = (SELECT id_service FROM service WHERE nom = 'cardiologie');
...
Requête : SELECT s.nom 
          FROM service s
          JOIN chambre c ON s.id_service = c.id_service
          GROUP BY s.nom
          HAVING COUNT(c.id_chambre) > 5;
...
Requête : SELECT prenom FROM patient 
          WHERE poids > (SELECT AVG(poids) FROM patient);
...
Requête : SELECT prenom FROM personnel 
          WHERE id_service = (SELECT id_service FROM personnel WHERE prenom = 'kevin');
...
Requête : SELECT p.prenom, p.nom 
          FROM patient p
          JOIN association_patient_chambre apc ON p.id_patient = apc.id_patient
          WHERE apc.id_chambre = (SELECT id_chambre FROM association_patient_chambre 
                        WHERE id_patient = (SELECT id_patient FROM patient WHERE prenom = 'lucas'));
...
Requête : SELECT pa.prenom, s.nom 
          FROM patient pa
          JOIN association_patient_chambre apc ON pa.id_patient = apc.id_patient
          JOIN chambre c ON apc.id_chambre = c.id_chambre
          JOIN service s ON c.id_service = s.id_service
          WHERE DATEDIFF(apc.date_sortie, apc.date_entree) > 10;
...
Requête : SELECT DISTINCT p1.prenom
          FROM patient p1
...
Requête : SELECT p.prenom, COUNT(apc.id_chambre) AS nombre_de_chambres
          FROM patient p
          JOIN association_patient_chambre apc ON p.id_patient = apc.id_patient
          GROUP BY p.prenom;
...
Requête : SELECT prenom FROM patient WHERE date_sortie IS NULL;
